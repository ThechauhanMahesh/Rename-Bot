...
async def rename_handler(c: Client, m: Message):
    ...
    ids = (m.reply_to_message.text).split(" ")
    file_p = await c.get_messages(int("-100" + ids[0]) , int(ids[1]))
    # Proceed
    ...
    if user_input_msg.text and user_input_msg.text.startswith("/"):
        await editable.edit("Process Cancelled!")
        return await user_input_msg.continue_propagation()
    _raw_file_name = get_media_file_name(file_p)
    if not _raw_file_name:
        _file_ext = mimetypes.guess_extension(get_file_attr(file_p).mime_type)
    ...
    is_big = get_media_file_size(file_p) > (10 * 1024 * 1024)
    if not is_big:
        _default_thumb_ = await db.get_thumbnail(m.from_user.id)
        if not _default_thumb_:
            _m_attr = get_file_attr(file_p)
            _default_thumb_ = _m_attr.thumbs[0].file_id \
                if (_m_attr and _m_attr.thumbs) \
                else None
        await handle_not_big(c, m, get_media_file_id(file_p), file_name,
                             editable, get_file_type(file_p), _default_thumb_)
        return
    file_type = get_file_type(file_p)
    _c_file_id = FileId.decode(get_media_file_id(file_p))
    try:
        c_time = time.time()
